...
public class JVMThreadExtractor extends JVMDataExtractor<ThreadInfo> {
	...
	public JVMThreadExtractor(ProxyClient proxyClient, int jmxPort, String jmxProcess)
			throws IOException {
	...
	public List<FalconItem> build(ThreadInfo jmxResultData) throws Exception {
		...
		threadNumItem.setTags(StringUtils.lowerCase(tag));	
		...
		peakThreadNumItem.setTags(StringUtils.lowerCase(tag));	
